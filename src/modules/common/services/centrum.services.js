const sharedConstants = require("shared/constants");
const dbLogger = require("shared/services/dbLogger.services");
const axios = require("axios");

const centrumServices = {};

// @service-name : get document details
// @service-desc : get the document details from centrum
centrumServices.getDocuments = async (customerRefId) => {
  let documentDetails;

  // Note: change the url to actual URL
  const requestUrl = `http://api.centrum.com/v2`;
  const payload = JSON.stringify({
    customer_id: customerRefId,
  });
  const headers = {
    "Content-Type": "application/json",
  };

  // const res = await axios.post(requestUrl, payload, { headers });

  const res = {
    status: 200,
    result: [
      {
        proofType: "PROOF_OF_ADDRESS",
        docSide: "FRONT",
        docType: "AADHAAR",
        docData:
          "",
      },
      {
        proofType: "PROOF_OF_ADDRESS",
        docSide: "BACK",
        docType: "AADHAAR",
        docData:
          "",
      },
    ],
  };

  if (res.status == 200) {
    await dbLogger.log({
      action:
        sharedConstants.masterConstants.ACTION.CENTRUM_GET_CUSTOMER_DOCUMENTS,
      request: {requestUrl, headers, payload: JSON.parse(payload)},
      response: res,
      code: res.status,
    });

    documentDetails = res.result;
    return documentDetails;
  }

  if (res.status !== 200) {
    await dbLogger.log({
      action:
        sharedConstants.masterConstants.ACTION.CENTRUM_GET_CUSTOMER_DOCUMENTS,
      request: {requestUrl, headers, payload: JSON.parse(payload)},
      response: res,
      code: res.status,
    });

    return sharedServices.error.throw(
      sharedConstants.masterConstants.CENTRUM_ERROR.CENE0001
    );
  }
};

// @service-name : getCustomerPhoto
// @service-desc : get the photo from centrum
centrumServices.getCustomerPhoto = async (customerRefId) => {
  let customerPhoto;

  // Note: change the url to actual URL
  const requestUrl = `http://api.centrum.com/v2`;
  const payload = JSON.stringify({
    customer_id: customerRefId,
  });
  const headers = {
    "Content-Type": "application/json",
  };

  // const res = await axios.post(requestUrl, payload, { headers });

  const res = {
    status: 200,
    result: {
      customer_photo: "photo in base64 here",
    },
  };

  if (res.status == 200) {
    await dbLogger.log({
      action: sharedConstants.masterConstants.ACTION.CENTRUM_GET_CUSTOMER_PHOTO,
      request: {requestUrl, payload: JSON.parse(payload), headers},
      response: res,
      code: res.status,
    });

    customerPhoto = res.result.customer_photo;
    return customerPhoto;
  }

  if (res.status !== 200) {
    await dbLogger.log({
      action: sharedConstants.masterConstants.ACTION.CENTRUM_GET_CUSTOMER_PHOTO,
      request: {requestUrl, payload: JSON.parse(payload), headers},
      response: res,
      code: res.status,
    });

    return sharedServices.error.throw(
      sharedConstants.masterConstants.CENTRUM_ERROR.CENE0002
    );
  }
};

module.exports = centrumServices;
