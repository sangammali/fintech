...
// Api  : /fintech/stockal/redirect
// Desc : Stockal redirect api should return
...
describe("Case: Postive", () => {
  ...
  before(async () => {
    response = await chai
      .request(server)
      .get("/fintech/stockal/redirect")
      .set({
        "Authorization": redirectConstants.TEST_CONSTANTS.VALID_TOKEN,
        "planid": redirectConstants.TEST_CONSTANTS.PLAN_ID
      });
  });
  ...
  it("Should have planid key in header", () => {
    response.request.header.should.have.property('planid');
  });
  ...
  it("RedirectionURI should not be empty", () => {
    assert.isNotEmpty(response.body.result.redirectionURI);
  });
...
describe("Case : Invalid token", () => {
  ...
  it("Should have plan id key in header", () => {
    response.request.header.should.have.property('planid');
  });
